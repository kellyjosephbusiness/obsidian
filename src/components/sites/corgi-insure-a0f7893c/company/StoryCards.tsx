...
export function StoryCards({ stories }: { stories: CustomerStory[] }) {
  ...
  return (
    <div className="flex flex-col gap-8 md:gap-10">
          ...
          <li key={story.company} className="flex flex-col overflow-clip rounded-[24px] border border-[#e1e1e1] bg-white shadow-[0_0_24px_0_rgba(25,25,25,0.05)]">
            <div className="relative -mx-px -mt-px flex flex-col justify-end overflow-clip rounded-[24px] border border-[#e1e1e1] shadow-[0_0_24px_0_rgba(25,25,25,0.35)]">
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-5">
                ...
                <span className="shrink-0 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-medium uppercase leading-none tracking-[0.06em] text-[#4a4a4a] shadow-[0_0_8px_0_rgba(25,25,25,0.08)]">{story.industry}</span>
              </div>
            </div>
      ...
      {visible.length === 0 ? (
        <p className="flex items-center justify-center gap-2 text-body text-[#7b7b7b]">
