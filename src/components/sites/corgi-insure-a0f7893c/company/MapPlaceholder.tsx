      ...
      >
        ...
        {MAP_PLACEHOLDER.pins.map((pin) => (
          <div key={pin.city} aria-hidden className="absolute flex -translate-x-1/2 -translate-y-full flex-col items-center" style={{ left: `${pin.x}%`, top: `${pin.y}%` }}>
            <span className="mb-1 whitespace-nowrap rounded-full border border-[#e1e1e1] bg-white px-2.5 py-1 text-[12px] font-medium leading-none tracking-[-0.18px] text-[#191919] shadow-[0_0_8px_0_rgba(25,25,25,0.08)]">
              {pin.city}
            </span>
            <MaterialIcon name="location_on" size={24} fill={1} className="text-[#1e3a8a]" />
          </div>
        ))}
        <span className="absolute right-4 bottom-4 rounded-full bg-white/90 px-3 py-1 text-[12px] font-medium uppercase leading-none tracking-[0.08em] text-[#5c5c5c]">Illustrative map</span>
      </div>
