        ...
        <ol className="relative flex flex-col gap-8 lg:grid lg:grid-cols-5 lg:gap-6">
          ...
          {items.map((item, i) => (
            <li key={item.title} className="relative flex gap-5 lg:flex-col lg:gap-6">
              <span className="relative z-10 mt-px flex size-6 shrink-0 items-center justify-center rounded-full border-2 border-[#7FB6FE] bg-[#191919]">
                <span className="size-2 rounded-full bg-[#7FB6FE]" />
                <span className="sr-only">Step {i + 1}</span>
              </span>
              <div className="flex flex-col gap-2">
                <span className="text-[12px] font-medium uppercase leading-[1.2] tracking-[0.08em] text-[#7FB6FE]">{item.when}</span>
