 ...
 * 12px medium labels, 1px `#e1e1e1` borders, `rounded-[10px]`, navy focus ring, on a `#f9f9f9` card.
 */
    ...
    <label htmlFor={id} className="flex cursor-pointer items-start gap-3">
      <span className="relative mt-0.5 flex size-[18px] shrink-0">
        <input
          ...
          className="peer size-full cursor-pointer appearance-none rounded-[5px] border border-[#bdbdbd] bg-white outline-none transition-colors checked:border-[#1e3a8a] checked:bg-[#1e3a8a] focus-visible:shadow-[0_0_0_3px_rgba(30,58,138,0.25)]"
        />
...
export function FormCard({ children, className, as: Tag = "div" }: { children: ReactNode; className?: string; as?: "div" | "form" }) {
  return <Tag className={cn("flex flex-col gap-6 rounded-[24px] border border-[#e1e1e1] bg-[#f9f9f9] p-6 shadow-[0_0_24px_0_rgba(25,25,25,0.05)] md:p-8", className)}>{children}</Tag>;
}
...
export function SubmittedState({ title, body, action, secondary, children }: { title: string; body: string; action: CompanyCta; secondary?: CompanyCta; children?: ReactNode }) {
  return (
    <div role="status" aria-live="polite" className="flex flex-col items-center gap-6 py-6 text-center md:py-10">
      <span className="flex size-16 items-center justify-center rounded-full bg-[#dde4f6] text-[#1e3a8a]">
