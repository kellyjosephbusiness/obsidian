...
/**
 * Rebranded press thumbnail (source: a 1024x520 PNG with the gold corgi). Built in JSX so it scales
 * with the 520px column: every size is in container-query units of the card width.
 */
    ...
    <div className="relative w-full overflow-hidden rounded-[8px] border border-[#e1e1e1] bg-[#191919] lg:justify-self-end">
      <div className="@container relative aspect-[519/264] w-full overflow-hidden rounded-[8px] outline outline-1 outline-black/10">
        ...
        <div className="absolute top-[4cqw] left-[3cqw] flex flex-col">
          ...
          <span className="mt-[2.4cqw] ml-[1cqw] text-[2.4cqw] leading-none tracking-[-0.01em] text-[#a3a3a3]">
            {thumbnail.valuationLabel}
          </span>
        </div>

        <div className="absolute bottom-[3.2cqw] left-[3cqw] flex flex-col">
          <span className="text-[2.4cqw] leading-none tracking-[-0.01em] text-[#d6d6d6]">
            <strong className="font-bold text-white">{thumbnail.raiseLead}</strong>
            {thumbnail.raiseTail}
          </span>
          <span className="mt-[1.6cqw] font-mono text-[7.8cqw] leading-none font-medium tracking-[-0.02em] text-white uppercase">
            {thumbnail.leadInvestor}
          </span>
        </div>
      </div>
    </div>
...
/** Hero: back link + H1 / thumbnail grid (`pt-[170px]` clears the fixed announcement bar + nav). */
    ...
    <section className="w-full px-4 pt-[170px] pb-16 md:px-6 lg:px-16">
      <div className="mx-auto flex w-full max-w-[1136px] flex-col gap-9">
        <div>
          <Link
            href={HERO.backHref}
            className="inline-flex items-center gap-1 text-body font-medium tracking-tight text-[#191919] hover:text-[#1e3a8a]"
          >
