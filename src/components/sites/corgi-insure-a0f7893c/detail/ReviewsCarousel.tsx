...
/**
 * Rating card + horizontally scrolling review cards with prev/next and a progress bar.
 * Sits on a white band between the grey feature cards and the grey callout, keeping the
 * tint → white → grey alternation intact.
 */
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[300px_minmax(0,1fr)] md:gap-6">
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <div ref={trackRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {reviews.items.map((r) => (
              <article key={r.title + r.author} data-review className="flex w-[300px] shrink-0 snap-start flex-col gap-3 rounded-[8px] border border-[#e1e1e1] bg-white p-6">
                <Stars size={14} />
          ...
          <div className="flex items-center gap-4">
            <button type="button" aria-label="Previous reviews" onClick={() => step(-1)} disabled={atStart} className="flex size-11 shrink-0 items-center justify-center rounded-[8px] border border-[#e1e1e1] bg-white text-[#191919] transition-colors hover:bg-[#f0f0f0] disabled:cursor-default disabled:opacity-30 disabled:hover:bg-white">
            ...
            <button type="button" aria-label="Next reviews" onClick={() => step(1)} disabled={atEnd} className="flex size-11 shrink-0 items-center justify-center rounded-[8px] border border-[#e1e1e1] bg-white text-[#191919] transition-colors hover:bg-[#f0f0f0] disabled:cursor-default disabled:opacity-30 disabled:hover:bg-white">
