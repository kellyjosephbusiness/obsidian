...
/** Centered two-line heading + sub, then three tinted cards with a big icon, title and body. */
    ...
    <Frame inner="flex flex-col gap-10 md:gap-14">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {features.cards.map((card) => (
          <article key={card.title} className="group flex flex-col items-center gap-6 rounded-[24px] bg-[#dde4f6] p-8 text-center md:p-10">
            <div className="flex size-[120px] items-center justify-center rounded-full bg-white text-[#1e3a8a] shadow-[0_0_24px_0_rgba(25,25,25,0.06)] transition-transform duration-300 group-hover:scale-105">
              <MaterialIcon name={card.icon} size={56} />
            </div>
